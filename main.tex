...
\begin{equation}
  ...
  \label{eq:co2_gamma}
\end{equation}
We adopt the IPCC AR6 best-estimate equilibrium climate sensitivity $\text{ECS} = 3.0\,^\circ\text{C}$ per \CO{} doubling \citep{Forster2021IPCCAR6Ch7}, a polar ocean-surface amplification factor $\alpha_\text{sst} = 1.5$, and a pre-industrial Atlantic equator-to-pole SST difference $\Delta T_0 = 30\,^\circ\text{C}$ (approximate Levitus climatology value). The Arctic sea-surface temperature response, which is the quantity relevant for the circulation-driving temperature field, is far weaker than the frequently quoted surface-\emph{air} amplification: it is strongly damped by sea-ice insulation and local ocean heat uptake, and is of order $1.5$ times the global-mean SST warming \citep{Meredith2019PolarRegions}. These choices give $\gamma = 0$ at pre-industrial \CO{} ($C_0 = 280\,\text{ppm}$) and $\gamma = 0.05$ at doubled \CO{} ($560\,\text{ppm}$). The parameter sweep covers seven uniformly spaced values of $\gamma \in [0,0.06]$ (steps of $0.01$), corresponding to \CO{} concentrations from $280\,\text{ppm}$ to approximately $643\,\text{ppm}$.
